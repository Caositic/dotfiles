<div>FM: Thunar</div>
<div>Text Editor: nano</div>
<div>Bar: Polybar</div>
<div>Program Launcer: Rofi</div>
<div>Browser: Betterfox/chrome</div>
<div>Archiving Tool: Ark</div>
<div>File Reader: Kate/VSC</div>
<div>Terminal: alacritty</div>
<div>Appearance: lxapperance</div>
<div>Compositor: Picom</div>
<div>Image Reader: sxiv</div>
<div>Wallpaper: Nitrogen</div>
<div>Multimedia: mpv</div>
<div>Additional information:</div>
<p>codex;wine,winegui;rofi-calc,powermenu;obs;grubtheme-particle;prismlauncher;lmstudio;xfce4-polkit;nerdfonts;Pinta;obs</p>
